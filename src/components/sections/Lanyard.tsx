...
interface GLTFResult {
  ...
  materials: Record<string, THREE.MeshPhysicalMaterial | THREE.MeshStandardMaterial>;
}
...
export default function Lanyard({
  ...
  useEffect(() => {
  // Hanya jalankan pengecekan layar setelah komponen selesai di-render di browser
  ...
  return (
    <div className="relative w-full h-125 flex justify-center items-center transform scale-100 origin-center touch-none">
      <Canvas
        camera={{ position, fov }}
...
function Band({
  ...
  // Mengakses file mentah secara publik sesuai standar Next.js
  const { nodes, materials } = useGLTF('/card.glb') as unknown as GLTFResult;
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && band.current) {
      ...
      const meshGeometry = band.current.geometry as any;
      if (meshGeometry && typeof meshGeometry.setPoints === 'function') {
        meshGeometry.setPoints(curve.getPoints(isMobile ? 16 : 32));
      }
